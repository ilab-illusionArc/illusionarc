...
useHead({ title: 'Admin · Messages — illusion Arc' })

type MsgStatus = 'new' | 'seen' | 'done'
...
const sb: any = supabase // ✅ avoids "never" typing if you didn't generate Database types
...
const q = ref('')
const statusFilter = ref<'all' | MsgStatus>('all')
...
const filtered = computed(() => {
  ...
  if (query) {
    list = list.filter(r => {
      const hay =
        `${r.name} ${r.email} ${r.subject || ''} ${r.message} ${r.project_type || ''} ${r.budget || ''}`.toLowerCase()
...
function badgeColor(s: MsgStatus): 'primary' | 'warning' | 'success' {
...
async function load(): Promise<void> {
  ...
  try {
    const { data, error } = await sb
      ...
      .select('id, created_at, name, email, subject, message, source, status, user_agent, ip, user_id, project_type, budget')
      .order('created_at', { ascending: false })
      .limit(800)

    if (error) throw error
    rows.value = (data || []) as ContactMessageRow[]
  } catch (e: any) {
    toast.add({
...
async function updateStatus(id: string, status: MsgStatus, opts?: { silent?: boolean }): Promise<void> {
  saving.value = id
  try {
...
async function openRow(r: ContactMessageRow): Promise<void> {
  selected.value = r

  // auto-mark new -> seen when opening
  if (r.status === 'new') {
...
async function removeMessage(id: string): Promise<void> {
  ...
  try {
    ...
    toast.add({ title: 'Deleted', description: 'Message removed.', color: 'success' })
  } catch (e: any) {
    toast.add({
      title: 'Delete failed',
      description: String(e?.message || 'Unknown error'),
...
// ✅ safe wrappers for modal buttons (no "selected possibly null")
...
async function deleteSelected(): Promise<void> {
  if (!selected.value) return
  await removeMessage(selected.value.id)
}
  ...
  <div class="page">
    ...
    <div class="head">
      <div class="left">
        ...
        <div class="sub">View, triage, and resolve incoming messages.</div>
      </div>
    ...
    <!-- Filters -->
    <div class="toolbar">
      <UInput v-model="q" icon="i-heroicons-magnifying-glass" placeholder="Search name, email, subject, message…" class="w-full" />
    ...
    <div class="card">
      ...
      <div v-else class="tableWrap">
        <table class="table">
          ...
          <tbody>
            <tr v-for="m in filtered" :key="m.id" class="tr" @click="openRow(m)">
              <td>
                <UBadge :label="m.status" variant="soft" :color="badgeColor(m.status)" />
              </td>

              <td class="from">
                <div class="nm">{{ m.name }}</div>
                <div class="dim">{{ m.email }}</div>
              </td>
              ...
              <td class="metaCell">
                <div class="dim">{{ fmtTime(m.created_at) }}</div>
                <div class="dim">src: {{ m.source || 'website' }}</div>
              </td>

              <td class="rightCol" @click.stop>
                <div class="btns">
                  <UButton size="xs" variant="soft" :loading="saving === m.id" @click="() => updateStatus(m.id, 'seen')">
                    Seen
                  </UButton>
                  <UButton size="xs" color="primary" variant="solid" :loading="saving === m.id" @click="() => updateStatus(m.id, 'done')">
                    Done
                  </UButton>
                </div>
              </td>
      ...
      <div v-if="selected" class="modal">
        <div class="mHead">
          ...
          <div class="mActions">
            ...
            <UButton color="red" variant="soft" :loading="saving === selected.id" @click="deleteSelected">
              Delete
            </UButton>
          </div>
        </div>
        ...
        <div class="mBody">
          ...
          <div class="block">
            ...
            <div class="v">{{ selected.subject || '—' }}</div>
          </div>
          ...
          <div class="grid2">
            <div class="block">
              ...
              <div class="v mono">{{ selected.user_id || '—' }}</div>
            </div>
            <div class="block">
              <div class="k">IP</div>
